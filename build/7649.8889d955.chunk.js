"use strict";
(self["webpackChunkstrapi_elearning"] = self["webpackChunkstrapi_elearning"] || []).push([[7649],{
...
const useFindRedirectionLink = (slug) => {
  const [{ rawQuery }] = (0,dist.useQueryParams)();
...
const removePasswordFieldsFromData = (data, contentTypeSchema, componentSchema) => {
  const recursiveCleanData = (data2, schema) => {
    return Object.keys(data2).reduce((acc, current) => {
      const attrType = (0,dist.getType)(schema, current);
      const value = get_default()(data2, current);
      const component = (0,dist.getOtherInfos)(schema, [current, "component"]);
      const isRepeatable = (0,dist.getOtherInfos)(schema, [current, "repeatable"]);
...
es/* addMethod */.kM(es/* mixed */.nK, "defined", function() {
  return this.test("defined", dist.translatedErrors.required, (value) => value !== void 0);
});
es/* addMethod */.kM(es/* array */.IX, "notEmptyMin", function(min) {
  return this.test("notEmptyMin", dist.translatedErrors.min, (value) => {
      ...
      if (attribute.type === "component") {
        ...
        if (attribute.repeatable === true) {
          ...
          let componentSchema2 = es/* lazy */.Vo((value) => {
            ...
            if (min) {
              if (required) {
                baseSchema = baseSchema.min(min, dist.translatedErrors.min);
              ...
              baseSchema = baseSchema.min(1, dist.translatedErrors.required);
            }
            if (max) {
              baseSchema = baseSchema.max(max, dist.translatedErrors.max);
            }
            return baseSchema;
          });
      ...
      if (attribute.type === "dynamiczone") {
        ...
        if (min) {
          if (attribute.required) {
            dynamicZoneSchema = dynamicZoneSchema.test("min", dist.translatedErrors.min, (value) => {
            ...
            }).test("required", dist.translatedErrors.required, (value) => {
              if (options.isCreatingEntry) {
          ...
          dynamicZoneSchema = dynamicZoneSchema.test("required", dist.translatedErrors.required, (value) => {
            if (options.isCreatingEntry) {
              return value !== null || value !== void 0;
            }
            if (value === void 0) {
        ...
        if (max) {
          dynamicZoneSchema = dynamicZoneSchema.max(max, dist.translatedErrors.max);
        }
...
const createYupSchemaAttribute = (type, validations, options) => {
  ...
  if (type === "blocks") {
    schema = es/* mixed */.nK().test("isJSON", dist.translatedErrors.json, (value) => {
  ...
  if (type === "json") {
    schema = es/* mixed */.nK(dist.translatedErrors.json).test("isJSON", dist.translatedErrors.json, (value) => {
      if (options.isJSONTestDisabled) {
        return true;
      }
    ...
    }).nullable().test("required", dist.translatedErrors.required, (value) => {
  ...
  if (type === "email") {
    schema = schema.email(dist.translatedErrors.email);
  }
  ...
  Object.keys(validations).forEach((validation) => {
    ...
    if (!!validationValue || !isBoolean_default()(validationValue) && Number.isInteger(Math.floor(validationValue)) || validationValue === 0) {
      switch (validation) {
        case "required": {
          if (!options.isDraft) {
            if (type === "password" && options.isCreatingEntry) {
              schema = schema.required(dist.translatedErrors.required);
            }
            if (type !== "password") {
              if (options.isCreatingEntry) {
                schema = schema.required(dist.translatedErrors.required);
              } else {
                schema = schema.test("required", dist.translatedErrors.required, (value) => {
        ...
        case "max": {
          if (type === "biginteger") {
            schema = schema.isInferior(dist.translatedErrors.max, validationValue);
          } else {
            schema = schema.max(validationValue, dist.translatedErrors.max);
          }
          break;
        }
        case "maxLength":
          schema = schema.max(validationValue, dist.translatedErrors.maxLength);
          break;
        case "min": {
          if (type === "biginteger") {
            schema = schema.isSuperior(dist.translatedErrors.min, validationValue);
          } else {
            schema = schema.min(validationValue, dist.translatedErrors.min);
          }
          break;
        }
        case "minLength": {
          if (!options.isDraft) {
            schema = schema.min(validationValue, dist.translatedErrors.minLength);
          }
          break;
        }
        case "regex":
          schema = schema.matches(new RegExp(validationValue), {
            message: dist.translatedErrors.regex,
